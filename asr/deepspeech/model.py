...
class DeepSpeechModel:
    ...
    def __setup_networks(self):
        # setup networks
        self.encoder = DeepSpeech(num_classes=self.y_dim)
        if self.use_cuda:
            self.encoder.cuda()
        # setup loss
        ...
        if self.opt == "adam":
            ...
            self.lr_scheduler = torch.optim.lr_scheduler.CosineAnnealingWithRestartsLR(self.optimizer, T_max=50, T_mult=2)
