...
class DenseNet(nn.Module):
    ...
    def __init__(self, growth_rate=32, block_config=(6, 12, 24, 16),
        ...
        #])
        self.hidden = nn.Sequential(
            nn.Conv2d(2, num_init_features, kernel_size=(41, 11), stride=(2, 2), padding=(0, 5)),
            nn.BatchNorm2d(num_init_features),
            #nn.ReLU(inplace=True),
            #nn.Hardtanh(0, 20, inplace=True),
            Swish(inplace=True),
            nn.Conv2d(num_init_features, num_init_features, kernel_size=(21, 11), stride=(1, 1), padding=(0, 5)),
            nn.BatchNorm2d(num_init_features),
            #nn.ReLU(inplace=True),
            #nn.Hardtanh(0, 20, inplace=True)
            Swish(inplace=True),
...
if __name__ == "__main__":
    ...
    print("densenet")
    net = densenet_custom(num_classes=p.NUM_CTC_LABELS)
